...
namespace PyrusApiClient
{
    public class PyrusClient
    {
        ...
        public async Task<TaskResponse> CommentTask(int taskId, TaskCommentRequest comment, string accessToken = null, Guid? idempotencyKey = null)
        {
            if (accessToken != null)
                Token = accessToken;
        ...
        public async Task<SyncCatalogResponse> UpdateCatalogItems(int catalogId, UpdateCatalogItemsRequest request, string accessToken = null)
        {
            Token = accessToken ?? Token;
        ...
        public async Task<RoleResponse> DeleteRole(DeleteRoleRequest request, string accessToken = null)
        {
            Token = accessToken ?? Token;
            var response = await this.RunQuery<RoleResponse>(() => RequestHelper.DeleteRequest(this, $"{ClientSettings.Origin}{RolesEndpoint}/{request.Id}", request, Token));
            return response;
        }
        ...
        public Task<InboxResponse> GetInbox(int tasksCount = 50, int groupTasksCount = 50, string accessToken = null) =>
            GetInbox(new InboxRequest() { TasksCount = tasksCount, GroupTasksCount = groupTasksCount }, accessToken);

        public Task<InboxResponse> GetInbox(int tasksCount = 50, string accessToken = null) =>
            GetInbox(tasksCount, 50, accessToken);

        public Task<InboxResponse> GetInbox(int tasksCount = 50) =>
            GetInbox(tasksCount, 50, null);

        public Task<InboxResponse> GetInbox() =>
            GetInbox(50, 50, null);
